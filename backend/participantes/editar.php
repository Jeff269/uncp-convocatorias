<?php
    ...
    if($parset_token->code=="200" && $parset_token->privilegios=="1"){
        $resultadoBD = false;
        $query = "UPDATE `participantes` SET `nro`='$data->nro', `nombres`=UPPER('$data->nombres'), `apellidos`=UPPER('$data->apellidos'), `participacion`=UPPER('$data->participacion'), `codigo`=UPPER('$data->codigo') WHERE `idparticipante`='$data->id';";
    }else{
