    ...
    if(substr($response,2,5)==="error"){
        ...
        if(count($json0)>=1){
            ...
            if(count($json)>=1){
                ...
                $query = "INSERT INTO `usuarios` (`privilegios`, `sub`, `email`, `name`, `family_name`, `given_name`) VALUES ('0', '$parse->sub', '$parse->email','$parse->name', '$parse->family_name', '$parse->given_name');";
                $result = $mysqli->query($query);
                if(!$result){
                    die("Query error " . mysqli_error($mysqli));
                }else{
