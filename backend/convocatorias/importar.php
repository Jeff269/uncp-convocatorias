<?php
    ...
    if(/*$parset_token->code=="200" && $parset_token->privilegios=="1"*/true){
        $resultadoUP = false;
        $resultadoBD = false;
        $data_csv_nro = [];
        $data_csv_nombres = [];
    ...
    $consulta = "";
    if($resultadoUP){
        for($i=1;$i< count($data_csv_facultad); $i++){
            $consulta .= "('$data_csv_nro[$i]','$data_csv_nombres[$i]','$data_csv_apellidos[$i]','$data_csv_participacion[$i]','$data_csv_codigo[$i]'),";
        }
        $consulta = substr($consulta, 0, -1);
        $query = "INSERT INTO certificados(nro,nombres,apellidos,participacion,codigo) VALUES $consulta;"; 
