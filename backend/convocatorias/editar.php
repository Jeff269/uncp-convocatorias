<?php
    ...
    if($parset_token->code=="200" && $parset_token->privilegios=="1"){
        $resultadoBD = false;
        $query = "UPDATE `certificados` SET `curso`=UPPER('$data->curso'),`codigo`=UPPER('$data->codigo'),`fecha_inicio`='$data->fecha_inicio',`fecha_fin`='$data->fecha_fin',`horas`='$data->horas',`year`='$data->year',`carpeta`=UPPER('$data->carpeta'),`post_ruta`='$data->post_ruta' WHERE `idcertificado`='$data->id';";
    }else{
