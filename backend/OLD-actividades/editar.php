<?php
    ...
    if($parset_token->code=="200" && $parset_token->privilegios=="1"){
        $resultadoBD = false;
        $query = "UPDATE `actividades` SET `nombre_act`='$data->nombre_act',  `plazo`='$data->plazo',  `financiamiento`='$data->financiamiento',  `tipo_proceso`='$data->tipo_proceso', `fecha_mod_Act`=now(), `proyectos_idproyecto`='$data->idproyecto' WHERE `idactividad`='$data->id';";
    }else{
