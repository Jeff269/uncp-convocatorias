<?php
    ...
    if($data->iddependencia==null || $data->iddependencia=='NULL'){
        $data->iddependencia = "";
    }
    ...
    if($parset_token->code=="200" && $parset_token->privilegios=="1"){
        ...
        if(isset($data->iddependencia) && $data->iddependencia!=""){ 
            $query = "UPDATE `usuarios` SET `privilegios`='$data->privilegios', `dependencias_iddependencia`='$data->iddependencia' WHERE `idusuario`='$data->id';";
        }else{
            $query = "UPDATE `usuarios` SET `privilegios`='$data->privilegios' WHERE `idusuario`='$data->id';";
        }
