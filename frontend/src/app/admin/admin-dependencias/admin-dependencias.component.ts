...
@Component({
  selector: 'app-admin-dependencias',
  templateUrl: './admin-dependencias.component.html',
  styleUrls: ['./admin-dependencias.component.css']
})
export class AdminDependenciasComponent implements OnInit {
  ...
  constructor(private service: DependenciasService, private toastr: ToastrService) { 
    this.dependencias = [];
  ...
  paginar(){
    ...
    this.service.getPages(this.pageSize).subscribe(
      res => {
        //console.log(res)
        this.pages = res;
        this.getDependencias(this.pageActive);
      },
      err => {console.log(err)}
    );
  }
  ...
  edit(getElement: Dependencia){
    this.dependencia = Object.assign({},getElement);
  }
  ...
  submit(getForm){
    if(this.dependencia.id){ 
      this.btn = false;
      this.service.putData(this.dependencia).subscribe(
        (res) => {
          if(res==200){
      ...
      this.btn = false;
        this.service.postData(this.dependencia).subscribe(
          (res) => {
            if(res==200){
  ...
  eliminar(getElement: any){  
    if(getElement.id){
      ...
      this.btn = false;
      this.service.deleteData(this.dependencia).subscribe(
        res => {
          if(res==200){
            this.btn = true;
